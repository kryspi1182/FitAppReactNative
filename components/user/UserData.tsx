...
const UserData: React.FC = () => {
  ...
  const mappedTrainingConditions = trainingConditions.map(
    (trainingCondition) => {
  ...
  const [selectedMedicalConditions, setSelectedMedicalConditions] =
    React.useState(user.medicalConditions);
  const [selectedUnwantedProducts, setSelectedUnwantedProducts] =
    React.useState(user.unwantedProducts);
  const [selectedTrainingConditions, setSelectedTrainingConditions] =
    React.useState(user.trainingConditions);
  const [selectedMappedMedicalConditions, setSelectedMappedMedicalConditions] =
    React.useState(
      user.medicalConditions
        ? mappedMedicalConditions.filter((mapped) =>
            user.medicalConditions.some(
              (med) => med.medicalConditionId === mapped.id
            )
          )
        : ([] as AutocompleteItem[])
    );
  const [selectedMappedUnwantedProducts, setSelectedMappedUnwantedProducts] =
    React.useState(
      user.unwantedProducts
        ? mappedProducts.filter((mapped) =>
            user.unwantedProducts.some((prod) => prod.productId === mapped.id)
          )
        : ([] as AutocompleteItem[])
    );
  const [
    ...
    user.trainingConditions
      ? mappedTrainingConditions.filter((mapped) =>
          user.trainingConditions.some(
            (cond) => cond.trainingConditionId === mapped.id
          )
        )
      : ([] as AutocompleteItem[])
  );
  const mapItemsToMedicalConditions = (items: AutocompleteItem[]) => {
    ...
    const medCons = medicalConditions.filter((medCon) =>
      items.some((item) => item.id === medCon.id)
    );
    const userMedCons = medCons.map((medCon) => {
      return {
        userId: user.id,
        medicalConditionId: medCon.id,
      } as UserMedicalCondition;
    });
  ...
  const mapItemsToUnwantedProducts = (items: AutocompleteItem[]) => {
    ...
    const unProds = products.filter((prod) =>
      items.some((item) => item.id === prod.id)
    );
  ...
  const mapItemsToTrainingConditions = (items: AutocompleteItem[]) => {
    ...
    const conditions = trainingConditions.filter((condition) =>
      items.some((x) => x.id === condition.id)
    );
    const userTrainingConditions = conditions.map((cond) => {
      return {
        userId: user.id,
        trainingConditionId: cond.id,
      } as UserTrainingCondition;
    });
  ...
  const [weightTargetId, setWeightTargetId] = React.useState(
    user.weightTargetId
  );
  ...
  React.useEffect(() => {
    if (gender === 1) setGenderText("Male");
    else if (gender === 2) setGenderText("Female");
  ...
  const formik = useFormik({
    ...
    validationSchema: Yup.object({
      age: Yup.number()
        .min(
          10,
          "Little children should have their diets checked by an expert."
        )
        .max(130, "Are you sure you're that old?")
        .required("Providing your age is mandatory"),
      activity: Yup.number().required("Providing your activity is mandatory"),
      weight: Yup.number()
      ...
      gender: Yup.number().required("Providing your gender is mandatory"),
    }),
    ...
    <ScrollView contentContainerStyle={styles.container}>
      ...
      <View style={styles.row}>
        <View style={styles.input}>
          <Text>Age:</Text>
          <NumberInput value={age} setValue={setAge} />
      ...
      <View style={styles.row}>
        ...
        <View style={styles.input}>
          ...
          >
            <Picker.Item
              label="Lose weight"
              value={WeightTargetEnum.LoseWeight}
            />
            <Picker.Item
              label="Maintain weight"
              value={WeightTargetEnum.MaintainWeight}
            />
            <Picker.Item
              label="Gain weight"
              value={WeightTargetEnum.GainWeight}
            />
          </Picker>
          <Divider style={styles.divider} />
        </View>

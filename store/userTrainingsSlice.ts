...
export const fetchMatchingTrainings = createAsyncThunk(
  "training/match",
  async (params: UserTrainingParams) => {
    try {
...
export const fetchMatchingTrainingsUserData = createAsyncThunk(
  "training/match/user",
  async (params: UserTrainingParams) => {
    try {
...
export const resetTrainings = createAsyncThunk(
  "training/reset",
  async () => {}
);
...
} = userTrainingsAdapter.getSelectors(
  (state: RootState) => state.userTrainings
);
...
const userTrainingsSlice = createSlice({
  ...
  extraReducers: (builder) => {
    builder
      .addCase(
        fetchMatchingTrainings.fulfilled,
      ...
      .addCase(
        fetchMatchingTrainingsUserData.fulfilled,
        (state, action: PayloadAction<Array<Training>>) => {
          if (action.payload) {
            userTrainingsAdapter.upsertMany(state, action);
          }
